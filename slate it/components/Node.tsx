...
const arePropsEqual = (prev: NodeProps, next: NodeProps) => {
    // 如果节点正在被拖动，允许更新（返回false表示需要重新渲染）
    if (next.isDragging && next.node.id === next.node.id) {
        // 检查位置是否变化
        if (prev.node.x !== next.node.x || prev.node.y !== next.node.y) {
            return false;
        }
    }
    ...
    // 比较节点数据
    if (JSON.stringify(prev.node.data) !== JSON.stringify(next.node.data)) {
        return false;
    }
...
const NodeComponent: React.FC<NodeProps> = ({ 
  node, onUpdate, onAction, onDelete, onExpand, onCrop, onNodeMouseDown, onPortMouseDown, onPortMouseUp, onNodeContextMenu, onMediaContextMenu, onResizeMouseDown, inputAssets, onInputReorder, availableAiModels, isDragging, isGroupDragging, isSelected, isResizing, isConnecting 
}) => {
  ...
  const [atMenuPosition, setAtMenuPosition] = useState({ x: 0, y: 0 });
  ...
  const getAvailableImages = useCallback(() => {
      const images: Array<{ src: string, index: number, label: string }> = [];
      
      // 从输入资产获取
      if (inputAssets && inputAssets.length > 0) {
          inputAssets.forEach((asset, idx) => {
              if (asset.type === 'image') {
                  images.push({ src: asset.src, index: idx, label: `Image ${idx + 1}` });
              }
          });
      }
  ...
  const filteredImages = useMemo(() => {
      ...
      return availableImages.filter(img => 
          img.label.toLowerCase().includes(atSearchQuery.toLowerCase())
      );
  }, [availableImages, atSearchQuery]);
  ...
  const handlePromptChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      if (lastAtIndex !== -1) {
          ...
          // 如果 @ 后面没有空格或换行，且不是完整的 @Image N 格式，显示菜单
          if (!textAfterAt.includes(' ') && !textAfterAt.includes('\n') && !textAfterAt.match(/^Image \d+$/)) {
              ...
              if (textarea) {
                  ...
                  const atIndexInLine = lineText.lastIndexOf('@');
                  
                  // 使用字符宽度估算（对于等宽字体更准确，但也能工作）
                  const charWidth = 6; // 估算字符宽度
                  const atX = atIndexInLine * charWidth;
                  
                  setAtSearchQuery(textAfterAt);
                  setAtMenuPosition({
                      x: rect.left + paddingLeft + atX,
                      y: rect.top + paddingTop + (currentLine * lineHeight) + lineHeight
                  });
                  // 直接设置显示菜单，不检查图片数量
                  setShowAtMenu(true);
                  return;
  ...
  // 插入 @ 引用
  const insertAtReference = (assetIndex: number, assetSrc: string) => {
      const textarea = textareaRef.current;
      if (!textarea) return;
      
      const cursorPos = textarea.selectionStart;
      const textBeforeCursor = localPrompt.substring(0, cursorPos);
      const lastAtIndex = textBeforeCursor.lastIndexOf('@');
      
      if (lastAtIndex !== -1) {
          const textAfterAt = textBeforeCursor.substring(lastAtIndex + 1);
          const newText = localPrompt.substring(0, lastAtIndex) + `@Image ${assetIndex + 1} ` + localPrompt.substring(cursorPos);
          setLocalPrompt(newText);
          setShowAtMenu(false);
          
          // 恢复光标位置
          setTimeout(() => {
              const newCursorPos = lastAtIndex + `@Image ${assetIndex + 1} `.length;
  ...
  const handleDownload = (e: React.MouseEvent) => { e.stopPropagation(); const a = document.createElement('a'); a.href = node.data.image || videoBlobUrl || node.data.audioUri || ''; a.download = `slate-it-${Date.now()}`; document.body.appendChild(a); a.click(); document.body.removeChild(a); };
      ...
      if (file) { 
          ...
          reader.onload = (e) => {
              ...
              video.onloadedmetadata = () => {
                  ...
                  // 计算视频在节点宽度下的实际显示高度，使画框紧贴视频边缘
                  const availableWidth = nodeWidth;
                  const displayHeight = availableWidth / aspectRatio;
                  onUpdate(node.id, { videoUri: base64 }, { height: displayHeight });
              };
              video.src = base64;
          }; 
      ...
      if (file) { 
          ...
          reader.onload = (e) => {
              ...
              img.onload = () => {
                  ...
                  // 计算图片在节点宽度下的实际显示高度，使画框紧贴图片边缘
                  const availableWidth = nodeWidth;
                  const displayHeight = availableWidth / aspectRatio;
                  onUpdate(node.id, { image: base64 }, { height: displayHeight });
              };
              img.src = base64;
          }; 
          reader.readAsDataURL(file); 
  ...
  const renderTopBar = () => {
    ...
    // 图片节点使用更近的距离，视频节点保持原距离
    const topOffset = node.type.includes('IMAGE') ? '-top-10' : '-top-20';
    return (
    <div className={`absolute ${topOffset} left-0 w-full flex items-center justify-between px-1 transition-all duration-300 z-[200] ${showTopBar ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 pointer-events-none'}`}>
  ...
  const renderMediaContent = () => {
          ...
          return (
            <div className="w-full h-full p-5 flex flex-col gap-3">
                 <div className="relative w-full h-32 rounded-xl bg-black/20 border border-white/5 overflow-hidden flex items-center justify-center cursor-pointer hover:bg-black/30 transition-colors group/upload" onClick={() => !node.data.videoUri && fileInputRef.current?.click()}>
                    {videoBlobUrl ? <video src={videoBlobUrl} className="block w-full h-auto opacity-80" muted onMouseEnter={safePlay} onMouseLeave={safePause} onClick={handleExpand} onLoadedMetadata={(e) => {
                        // 检测视频实际尺寸并更新节点高度
                        const video = e.target as HTMLVideoElement;
                        if (video.videoWidth && video.videoHeight) {
                            const aspectRatio = video.videoWidth / video.videoHeight;
                            const newHeight = (nodeWidth - 48) / aspectRatio + 48;
                            if (Math.abs(newHeight - nodeHeight) > 5) {
                                onUpdate(node.id, {}, { height: newHeight });
                            }
                        }
                    }} /> : <div className="flex flex-col items-center gap-2 text-slate-500 group-hover:upload:text-slate-300"><Upload size={20} /><span className="text-[10px] font-bold uppercase tracking-wider">上传视频</span></div>}
      ...
      return (
        <div className={`w-full relative group/media ${hasContent ? '' : 'aspect-video bg-zinc-900'}`} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} style={hasContent ? { height: 'fit-content', minHeight: '0', display: 'block' } : {}}>
            {!hasContent ? (
                ...
                <>
                    {node.data.image ? 
                        <img ref={mediaRef as any} src={node.data.image} className="block w-full h-auto transition-transform duration-700 group-hover/media:scale-105" draggable={false} style={{ filter: showImageGrid ? 'blur(10px)' : 'none' }} onContextMenu={(e) => onMediaContextMenu?.(e, node.id, 'image', node.data.image!)} onLoad={(e) => {
                            // 检测图片实际尺寸并更新节点尺寸，使画框紧贴图片边缘
                            ...
                            if (img.naturalWidth && img.naturalHeight) {
                                ...
                                const availableWidth = nodeWidth;
                                const displayHeight = availableWidth / aspectRatio;
                                // 节点高度 = 图片显示高度（紧贴边缘，无额外 padding）
                                const newHeight = displayHeight;
                                if (Math.abs(newHeight - nodeHeight) > 2) { // 只有差异大于2px才更新
                                    onUpdate(node.id, {}, { height: newHeight });
                                }
                            }
                        }} /> 
                    : 
                        <div className="relative">
                            ...
                            />
                            {/* 播放按钮 - 只在鼠标悬停时显示，根据播放状态显示播放或暂停图标，半透明设计 */}
                            {!showImageGrid && (
                                <div
                                    className="absolute inset-0 flex items-center justify-center transition-all duration-200 z-20 group/play opacity-0 group-hover/media:opacity-100 pointer-events-none"
                                >
                                    <button
                                        onClick={toggleVideoPlay}
                                        onMouseDown={(e) => e.stopPropagation()}
                                        className="w-16 h-16 rounded-full bg-black/30 backdrop-blur-sm border border-white/10 flex items-center justify-center hover:scale-110 hover:bg-black/40 transition-all shadow-lg group-hover/play:border-white/20 pointer-events-auto"
                                    >
                                        {isVideoPlaying ? (
                    ...
                    {showImageGrid && (node.data.images || node.data.videoUris) && (
                        <div className="absolute inset-0 bg-black/40 z-10 grid grid-cols-2 gap-2 p-2 animate-in fade-in duration-200">
                            {node.data.images ? node.data.images.map((img, idx) => (
                                <div key={idx} className={`relative rounded-lg overflow-hidden cursor-pointer border-2 bg-zinc-900 ${img === node.data.image ? 'border-white/70' : 'border-transparent hover:border-white/50'}`} onClick={(e) => { e.stopPropagation(); onUpdate(node.id, { image: img }); }}>
                                    <img src={img} className="w-full h-full object-contain" />
                                </div>
                            )) : node.data.videoUris?.map((uri, idx) => (
                                <div key={idx} className={`relative rounded-lg overflow-hidden cursor-pointer border-2 bg-zinc-900 ${uri === node.data.videoUri ? 'border-white/70' : 'border-transparent hover:border-white/50'}`} onClick={(e) => { e.stopPropagation(); onUpdate(node.id, { videoUri: uri }); }}>
                                    {uri ? (
                                        <SecureVideo src={uri} className="w-full h-full object-contain bg-zinc-900" muted loop autoPlay />
                                    ) : (
            ...
            <div className={`w-full rounded-[20px] p-1 flex flex-col gap-1 ${GLASS_PANEL} relative z-[100]`} onMouseDown={e => e.stopPropagation()} onWheel={(e) => e.stopPropagation()}>
                <div className="relative group/input bg-black/10 rounded-[16px]">
                    <div className="relative">
                        <textarea 
                            ref={textareaRef}
                            className="w-full bg-transparent text-xs text-slate-200 placeholder-slate-500/60 p-3 focus:outline-none resize-none custom-scrollbar font-medium leading-relaxed relative z-10" 
                            style={{ height: `${Math.min(inputHeight, 200)}px` }} 
                            placeholder={node.type === NodeType.AUDIO_GENERATOR ? "描述您想生成的音乐或音效..." : "描述您的修改或生成需求..."} 
                            value={localPrompt} 
                            ...
                            onBlur={(e) => { 
                                ...
                                if (!relatedTarget || !atMenuRef.current?.contains(relatedTarget)) {
                            ...
                            onKeyDown={(e) => {
                                ...
                                // 回车或空格关闭 @ 菜单
                                if ((e.key === 'Enter' || e.key === ' ') && showAtMenu && filteredImages.length > 0) {
                                    e.preventDefault();
                                    insertAtReference(filteredImages[0].index, filteredImages[0].src);
                                }
                            }} 
                        ...
                        />
                        {/* 渲染 @ 标签覆盖层 */}
                        <div className="absolute inset-0 p-3 pointer-events-none text-xs text-transparent whitespace-pre-wrap break-words overflow-hidden" style={{ height: `${Math.min(inputHeight, 200)}px` }}>
                            {localPrompt.split(/(@Image \d+)/g).map((part, idx) => {
                                if (part.match(/^@Image \d+$/)) {
                                    return (
                                        <span key={idx} className="inline-flex items-center gap-1 px-2 py-0.5 mx-0.5 rounded-md bg-cyan-500/20 text-cyan-300 pointer-events-auto">
                                            {part}
                                        </span>
                                    );
                                }
                    ...
                    {/* @ 菜单 - 使用 Portal 渲染到 body 以确保正确显示 */}
                    {showAtMenu && (
                        <div 
                            ref={atMenuRef}
                            className="fixed z-[9999] bg-[#1c1c1e] border border-white/10 rounded-xl shadow-2xl backdrop-blur-xl overflow-hidden min-w-[200px] max-h-[200px] overflow-y-auto"
                            style={{ 
                                top: `${atMenuPosition.y}px`, 
                                ...
                                {filteredImages.length > 0 ? (
                                    filteredImages.map((img, idx) => (
                                        <div
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                insertAtReference(img.index, img.src);
                                            }}
                                        >
                                            <img src={img.src} alt={img.label} className="w-8 h-8 rounded object-cover" />
                ...
                <div className="flex items-center justify-between px-2 pb-1 pt-1 relative z-20">
                    <div className="flex items-center gap-2">
                         {/* AIService Model Selector - Standard select dropdown */}
                         {availableAiModels && availableAiModels.length > 0 && (
                             <select
                                 className="nodrag"
                             ...
                             >
                                 ...
                                 {availableAiModels.map(m => {
                                     // 从 config.ts 获取模型的显示名称
                                     let displayName = m;
                                     if (node.type === NodeType.IMAGE_GENERATOR || node.type === NodeType.IMAGE_EDITOR) {
                                         // 图片模型：使用 ID 作为显示名称
                                         displayName = m;
                                     } else if (node.type === NodeType.VIDEO_GENERATOR || node.type === NodeType.VIDEO_ANALYZER) {
                                         // 视频模型：如果有 name 字段则使用，否则使用 ID
                                         const videoModel = modelsConfig.models.video[m as keyof typeof modelsConfig.models.video];
                                         if (videoModel && 'name' in videoModel && videoModel.name) {
                                             displayName = videoModel.name;
                                         } else {
                                             displayName = m;
                                         }
                                     }
                         ...
                         {/* Original Model Selector (fallback if no AIService models) */}
                         {(!availableAiModels || availableAiModels.length === 0) && (
  ...
  return (
    <div 
        ...
        className={`absolute rounded-[24px] group w-80 overflow-visible ${isSelected ? 'ring-1 ring-white/50 shadow-[0_0_40px_-10px_rgba(255,255,255,0.3)] z-30' : 'ring-1 ring-white/10 hover:ring-white/20 z-10'}`}
        style={{ 
            left: node.x, top: node.y, width: '320px', minWidth: '320px', maxWidth: '320px', height: 'fit-content', minHeight: '0',
    ...
    >
        {renderTopBar()}
        <div 
            className={`absolute -left-3 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border border-white/20 bg-[#1c1c1e] flex items-center justify-center transition-all duration-300 hover:scale-125 cursor-crosshair z-[100] shadow-md pointer-events-auto ${isConnecting ? 'ring-2 ring-white/60 animate-pulse' : ''}`} 
        ...
        >
            <Plus size={10} strokeWidth={3} className="text-white/50 pointer-events-none" />
        </div>
        <div 
            className={`absolute -right-3 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border border-white/20 bg-[#1c1c1e] flex items-center justify-center transition-all duration-300 hover:scale-125 cursor-crosshair z-[100] shadow-md pointer-events-auto ${isConnecting ? 'ring-2 ring-white/60 animate-pulse' : ''}`} 
        ...
        {/* --- 媒体容器：核心修复区域 --- */}
        <div
          className={`relative flex flex-col rounded-2xl overflow-hidden bg-zinc-950 border-2 transition-all ${isSelected ? 'border-blue-500 shadow-xl' : 'border-zinc-800'}`}
          style={{
            width: '320px',
            height: 'fit-content',
            minHeight: '0px',
